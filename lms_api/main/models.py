...
class Chapter(models.Model):
    ...
    def __str__(self):
        return self.title if self.title else f'Chapter {self.pk}'
...
class StudentStatusUpdate(models.Model):
    ...
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name='status_updates_student')

    def __str__(self):
        return f'Student Status Update for {self.student.fullname} - {self.time}'

    class Meta:
        verbose_name_plural = "18. Student Status Updates"

class TeacherStatusUpdate(models.Model):
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('COMPLETED', 'Completed'),
        ('IN_PROGRESS', 'In Progress'),
    ...
    time = models.DateTimeField(auto_now_add=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING')
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE, related_name='status_updates_teacher')

    def __str__(self):
        return f'Teacher Status Update for {self.teacher.full_name} - {self.time}'
