...
async def websocket_handler(websocket, path):
    while True:
        ...
        response_data = {'message': 'Received your message!'}
...
def remove_enrolled_student(request,course_id,student_id):
    student=models.Student.objects.filter(id=student_id).first()
    course=models.Course.objects.filter(id=course_id).first()
    removedStatus=models.StudentCourseEnrollment.objects.filter(course=course,student=student).delete()
    if removedStatus:
        return JsonResponse({'bool':True})
    else:
        return JsonResponse({'bool':False})
...
class StudentStatusUpdateListCreateView(generics.ListCreateAPIView):
    queryset = models.StudentStatusUpdate.objects.all()
    serializer_class = StudentStatusUpdateSerializer

class TeacherStatusUpdateListCreateView(generics.ListCreateAPIView):
    queryset = models.TeacherStatusUpdate.objects.all()
